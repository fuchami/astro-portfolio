...
Jupyter Notebookを触る場合にのみVScodeを使っていましたが、最近はNotebook環境も [marimo](https://marimo.io/) を使うようになり、NeoVimで完結出来るようになったため、ほとんど起動する機会はないです。
...
プライベートでは特にサブスクプランは契約していませんが検証も兼ねてローカルLLMを使うことはあります。
...
### キーボード
US配列のHHKBを使っています。
カッコつけて無刻印を購入したのですが、流石に数字入力が辛いので「6」の部分を青色キートップに変えています。
...
LGの32インチ4Kモニターを使っています。でかいモニタは正義です。

### デスク

ロウヤのダイニングテーブルをデスクを使っています。
...
GUIが苦手なのと、純粋にカッコよくて気分が上がるからという理由でCUI/TUIツールを多用しています。
